...
The Power BI connector uses the Couchbase ODBC driver to connect to these data sources.
Using the ODBC driver, you can create an ODBC Data Source Name (DSN) for each data source you want to use in Power BI.
...
Once the DSN is configured, the Power BI connector lets you import the data from the specified Couchbase data source into Power BI. 

== Prerequisites

If you're using Couchbase Server or Capella Operational as your data source, ensure that the Analytics Service is running on the target node.
...
TIP: To verify is the connector is installed correctly, open Power BI and go to menu:Get Data from Other Sources[Get data from another source]. Use the search box to find btn:[Couchbase Connector]. If the connector appears in the search results, then it is installed correctly.

== Install OpenSSL

Before install the Couchbase ODBC driver, ensure that OpenSSL is installed, as it is a required runtime dependency for the driver.
...
For Couchbase Server and Capella Operational, the ODBC driver connects to the xref:server:analytics:1_intro.adoc[Couchbase Analytics Service]. 
