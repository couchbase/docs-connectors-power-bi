...
NOTE: You can either use standard credentials or client certificate authentication to set up the ODBC connection. If you choose to use client certificate authentication, then you must fill in the `CertificateFile Path`, `Trust Store Path`, and `Client Private Key Path` fields, and leave the `User` and `Password` fields blank.
--
